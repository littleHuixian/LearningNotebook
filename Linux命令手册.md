...
在正式开始之前，建议首先检查系统软件是否为最新，并更新到最新版本。
...
说明：GCC编译器支持编译Go，Objective-C，Objective-C++，Fortran，Ada，D等程序；Linux开发C/C++一定要熟悉GCC；VSCode是通过调用GCC编译器来实现C/C++的编译工作的；

使用gcc指令编译c代码；使用g++指令编译C++代码。

打开一个终端，使用apt安装GCC编译器和调试器：
...
第一步在`linux`系统终端输入`sudo apt-get install ssh`安装`ssh`服务

![images](./images/linux_ssh_index1.png)

第二步在终端中输入`sudo service ssh start`开启`ssh`服务

<img src="./images/linux_ssh_index2.png" alt="images" style="zoom: 67%;" />

输入`ifconfig`查看`Linux`的ip信息

![images](./images/linux_ssh_index3.png)

连接linux

<img src="./images/linux_ssh_index4.png" alt="images" style="zoom:80%;" />

输入linux的用户名和密码

![images](./images/linux_ssh_index5.png)

在xshell中输入`ssh 192.168.xx.xxx`远程连接

<img src="./images/linux_ssh_index7.png" alt="images" style="zoom:80%;" />

## 1.4 linux装机必备

cmake、cmake-gui、vim、vscode、Qt、tree、git
...
| tree                     | 查看⽂件和⽬录的树形结构                                     |
| mkdir <⽬录名>           | make directory 创建⽬录                                      |
| mkdir dir1 dir2          | 同时创建两个⽬录                                             |
| mkdir -p /tmp/dir1/dir2  | make directory 创建⽬录树                                    |
| rm -f file1              | remove file 删除'file1'⽂件                                  |
| rmdir dir1               | remove directory 删除'dir1'⽬录                              |
| rm -rf dir1              | -rf表示强制删除， 删除'dir1'⽬录和其内容                     |
| rm -rf dir1 dir2         | 同时删除两个⽬录及其内容                                     |
| mv old_dir new_dir       | move 重命名/移动⽬录                                         |
| cp file1 file2           | copy 复制⽂件                                                |
